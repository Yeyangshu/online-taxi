...
import com.yeyangshu.internalcommon.dto.servicemap.datatransferobject.OrderRequest;
...
public class OrderTask implements ITask {
    ...
    public int execute(long current) {
        ...
        //判断是否已有司机接单
        if (order.getStatus() != DispatchConstant.ORDER_STATUS_RE_RESERVED && order.getStatus() != DispatchConstant.ORDER_STATUS_ORDER_START) {
            status = STATUS_END;
            return status;
        }
        ...
        try {
            ...
            for (Integer distance : taskCondition.getDistanceList()) {
                ...
                for (DriverData data : list) {
                    ...
                    log.info("#orderId= " + orderId + "  round = " + round + "车辆高德信息：" + JSONObject.parseObject(String.valueOf(amapVehicle)));
                    try {
                        ...
                        updateOrder.setFollowing(data.getIsFollowing());
                        ...
                        if (success) {
                            ...
                            //短信司机
                            String phone = EncryptUtil.decryptionPhoneNumber(data.getDriverInfo().getPhoneNumber());
                            ...
                            // 发送消息
                            DispatchService.ins().sendSmsMessage(phone, DispatchConstant.SMS_FORCE_DISPATCH_DRIVER, smsMap);
                            return;
                        }
                    } catch (Exception e) {
    ...
    protected String getTypeDesc(int serviceType, int isFollowing, int userFeature) {
        ...
        if (isFollowing == 1) {
            ...
            if (serviceType == OrderTypeEnum.NORMAL.getCode()) {
                if (userFeature == DispatchConstant.USER_FEATURE_CHILDREN) {
                    s = "儿童用车系统派单";
                } else if (userFeature == DispatchConstant.USER_FEATURE_WOMAN) {
                    s = "女性用车系统派单";
                } else {
                    s = "预约派单";
                }
