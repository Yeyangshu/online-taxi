...
public class DispatchService {
    ...
    public List<TaskCondition> getForceTaskCondition(String cityCode, int serviceTypeId, int round) {
        ...
        for (int n = 0; n < round; n++) {
            for (int i = 0; i < 6; i++) {
                ...
                distanceList.add(4);
                distanceList.add(6);
                TaskCondition taskCondition = new TaskCondition(carDispatchTimeThresholdSet.getCarServiceBeforeInterval(),
                        carDispatchTimeThresholdSet.getCarServiceAfterInterval(), 0, 20, Integer.MAX_VALUE,
                        distanceList, DispatchConstant.COMPARE_TYPE_1);
    ...
    public List<TaskCondition> getSpecialCondition(String cityCode, int serviceTypeId) {
        ...
        log.info("#order carDispatchDistributeRadiusSet " + JSONObject.parseObject(String.valueOf(carDispatchDistributeRadiusSet)).toString());

        TaskCondition taskCondition1 = new TaskCondition(carDispatchTimeThresholdSet.getCarServiceBeforeInterval(), carDispatchTimeThresholdSet.getCarServiceAfterInterval(), carDispatchDistributeRadiusSet.getMaxRadius(), 20, carDispatchDistributeRadiusSet.getMaxRadiusFirstPushDriverCount(), null, DispatchConstant.COMPARE_TYPE_2);
        TaskCondition taskCondition2 = new TaskCondition(carDispatchTimeThresholdSet.getCarServiceBeforeInterval(), carDispatchTimeThresholdSet.getCarServiceAfterInterval(), carDispatchDistributeRadiusSet.getMaxRadius(), 20, carDispatchDistributeRadiusSet.getMaxRadiusFirstPushDriverCount(), null, DispatchConstant.COMPARE_TYPE_2);
    ...
    public void sendSmsMessageHx(String phone, String code, String... content) {
        try {
            ...
            log.error("invoke sms service error");
        }
    }
    ...
    public ResponseResult dispatch(DispatchRequest dispatchRequest) {
        ...
        if (orderRulePrice == null) {
            return ResponseResult.fail(CommonStatusEnum.FAIL.getCode(), "订单orderRulePrice null");
        }
        ...
        if (carDispatchTimeThresholdSet == null) {
            return ResponseResult.fail(CommonStatusEnum.FAIL.getCode(), "carDispatchTimeThresholdSet null");
        }
        ...
        if (list == null) {
            return ResponseResult.fail(CommonStatusEnum.FAIL.getCode(), "订单 list null");
        }
    ...
    public List<DriverData> getCarByOrder(Order order, TaskCondition taskCondition, int distance, List<Integer> usedIds, int round, boolean searchType) {
        ...
        Timestamp startTime = new Timestamp(order.getOrderStartTime().getTime() - TimeUnit.MINUTES.toMillis(taskCondition.getFreeTimeBefore()));
        ...
        for (Vehicle data : response.getData().getVehicles()) {
            try {
                ...
                if (workStatus != DispatchConstant.DRIVER_WORK_STATUS_GET_ORDER && workStatus != DispatchConstant.DRIVER_WORK_STATUS_WORK && csWorkStatus != DispatchConstant.DRIVER_CS_WORK_STATUS_WORK) {
                ...
                if (taskCondition.getCompareType() > 0 && driverBaseInfo != null && driverBaseInfo.getAddressLatitude() != null && !driverBaseInfo.getAddressLatitude().isEmpty() && driverBaseInfo.getAddressLongitude() != null && !driverBaseInfo.getAddressLongitude().isEmpty()) {
                    ...
                    if (taskCondition.getCompareType() == DispatchConstant.COMPARE_TYPE_2) {
                    ...
                    } else if (taskCondition.getCompareType() == DispatchConstant.COMPARE_TYPE_1) {
    ...
    public boolean hasDriver2(String city, Date time, int carType, int serviceTypeId) {
        ...
        for (CarDispatchCapacitySet data : list) {
            ...
            JSONObject jsonObject = JSONObject.parseObject(data.getCarServicePeriod());
    ...
    public int getDateTimeType(Date date) {
        ...
        if (h >= DispatchConstant.DAY_TIME_START_NUM && h < DispatchConstant.DAY_TIME_END_NUM) {
            return DispatchConstant.DAY_TIME;
        } else {
            return DispatchConstant.NIGHT;
        }
    }
